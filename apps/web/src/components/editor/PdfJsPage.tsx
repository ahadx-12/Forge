...
import { buildElementId, normalizeBbox, type PdfJsRect, roundTo } from "@/components/editor/pdfJsGeometry";
...
export type PdfJsSelectionItem = Pick<
  PdfJsTextItem,
  "element_id" | "text" | "bbox" | "element_type" | "style"
>;
    ...
    async function renderPage() {
      ...
      const items = textContent.items
        ...
        .map((item) => {
          ...
          const bbox = normalizeBbox(rect, viewport.width, viewport.height);
          const element_id = buildElementId(pageIndex, rect, text);
    ...
    <div ref={containerRef} className="w-full" data-page-index={pageIndex}>
      ...
      >
        ...
        <div className="absolute inset-0">
          {textItems.map((item) => {
            const overlayEntry = overlayEntriesMap[item.element_id];
            const isSelected = selectedElementId === item.element_id;
            return (
              <div
                key={item.element_id}
                onClick={() => onSelect(item, overlayEntry)}
